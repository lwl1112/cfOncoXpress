...
def resmat(predmat,truthmat,samplename):
    mynewmatrix=pd.DataFrame(columns=['gene_id','gene_name', 'TPM', 'Prediction'])
    for i,row in predmat.iterrows():
        res=truthmat[(truthmat['IDENTIFIER']==row['gene_name']) & (truthmat['GENE_ID']==row['gene_id'])][samplename]
        if len(res)==1:
            tpm=res.values[0]
            newrow = {'gene_id':row['gene_id'], 'gene_name': row['gene_name'],'TPM':tpm, 'Prediction':row['Prediction']}
...
for samplename in mat.columns[1:-1].values:
    ...
    newmat.to_csv(f'{outdir}/{samplename}_pred.csv')  # with TPM
