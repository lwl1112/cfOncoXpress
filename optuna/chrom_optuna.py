...
OUTDIR = "optuna_leave_one_chromosome_out"
...
N_TRIALS_NN = 80
RANDOM_STATE = 42
TARGET = "log2TPM_plus1"
CHROM_COL = "chr"
CHROMS = [str(i) for i in range(1, 23)] + ["X"]

# =========================================================
# 1. Load controls and chromosome annotation
# =========================================================

control01 = pd.read_csv(
    "/athena/khuranalab/scratch/wel4007/csv_files/finalfeatures/redo/control01bigmat_all_rmdupgenes.csv"
)[use_cols]
control01["sample"] = "control01"

control02 = pd.read_csv(
    "/athena/khuranalab/scratch/wel4007/csv_files/finalfeatures/redo/control02bigmat_all_rmdupgenes.csv"
)[use_cols]
control02["sample"] = "control02"

wcmfc01 = pd.read_csv(
    "/athena/khuranalab/scratch/wel4007/csv_files/finalfeatures/redo/WCMFC01bigmat_all.csv"
)[use_cols]
wcmfc01["sample"] = "wcmfc01"

transcripts = pd.read_csv(
    "/athena/khuranalab/scratch/wel4007/gencode.hg19/finalTSS.rmchrM.txt",
    sep="\t",
    header=None,
)

# Extract gene name from field like ENST...:GENE...
transcripts["gene_name"] = transcripts[3].astype(str).str.split(":").str[1]

gene_chr_dict = (
    transcripts[["gene_name", 0]]
    .drop_duplicates("gene_name")
    .set_index("gene_name")[0]
    .to_dict()
)

# One combined dataframe, same as chrom3.py, but with sample name kept for outputs.
df = pd.concat([control01, control02, wcmfc01], ignore_index=True)
df[CHROM_COL] = df["gene_name"].map(gene_chr_dict)
df[CHROM_COL] = df[CHROM_COL].astype(str).str.replace("chr", "", regex=False)
df[TARGET] = np.log2(df["TPM"] + 1)

df = (
    df[["sample", "gene_name", CHROM_COL] + feature_cols + ["TPM", TARGET]]
    .replace([np.inf, -np.inf], np.nan)
    .dropna(subset=feature_cols + [TARGET, CHROM_COL])
    .copy()
)

# Keep only chromosomes used in the leave-one-chromosome-out CV.
...
def prepare_xy(input_df):
    clean_df = (
        input_df[feature_cols + [TARGET]]
        .replace([np.inf, -np.inf], np.nan)
        .dropna()
        .copy()
    )
...
def save_pareto_plot(study, model_name):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, f"{model_name}_pareto_front.png"), dpi=300)
    plt.close()


def save_best_params(study, model_name):
    # Multi-objective Optuna has a Pareto front. Here we select the Pareto trial
    # with the highest mean Spearman. If tied, choose the smaller MSE.
    best_trial = sorted(
        study.best_trials,
...
# =========================================================
# 4. Classical Optuna objective: leave one chromosome out
# =========================================================
...
def make_classical_objective(build_fn):
    def objective(trial):
        ...
        for heldout_chr in CHROMS:
            train_df = df[df[CHROM_COL] != heldout_chr]
            test_df = df[df[CHROM_COL] == heldout_chr]
        ...
        return float(np.mean(spearman_list)), float(np.mean(mse_list))
...
# =========================================================
# 6. NN Optuna objective: leave one chromosome out
# =========================================================
...
# =========================================================
# 7. Evaluate one fixed/best parameter set on each chromosome
# =========================================================

def evaluate_fixed_params(model_name, build_fn, params, is_nn=False):
    ...
    for heldout_chr in CHROMS:
        ...
        if is_nn:
            ...
            model = build_fn(fixed_trial, input_dim=X_train.shape[1])
        ...
        tmp = test_df[["sample", "gene_name", CHROM_COL, "TPM", TARGET]].copy()
        tmp["heldout_chr"] = heldout_chr
...
for model_name, build_fn in classical_builders.items():
    print(f"\nRunning Optuna leave-one-chromosome-out for {model_name}")
    ...
    study.optimize(make_classical_objective(build_fn), n_trials=N_TRIALS_CLASSICAL)
    ...
    metrics_df, pred_df = evaluate_fixed_params(
        model_name=model_name,
        build_fn=build_fn,
        params=best_trial.params,
        is_nn=False,
    )
    all_eval_metrics.append(metrics_df)
    all_eval_preds.append(pred_df)
...
for model_name, build_fn in nn_builders.items():
    ...
    study.optimize(make_nn_objective(build_fn), n_trials=N_TRIALS_NN)
...
# =========================================================
# 9. Save summaries and select final model
# =========================================================

best_summary_df = pd.DataFrame(all_best_trials).sort_values(
    by=["best_spearman", "best_mse"],
    ascending=[False, True],
)
best_summary_df.to_csv(os.path.join(OUTDIR, "all_model_best_summary.csv"), index=False)

metrics_chr_optuna_df = pd.concat(all_eval_metrics, ignore_index=True)
pred_chr_optuna_df = pd.concat(all_eval_preds, ignore_index=True)

metrics_chr_optuna_df.to_csv(os.path.join(OUTDIR, "metrics_chr_optuna_df.csv"), index=False)
pred_chr_optuna_df.to_csv(os.path.join(OUTDIR, "pred_chr_optuna_df.csv"), index=False)

chrom_summary_df = (
    metrics_chr_optuna_df
    .groupby("model")
    .agg(
...
chrom_summary_df.to_csv(os.path.join(OUTDIR, "chrom_optuna_summary.csv"), index=False)
...
print("Best Optuna mean Spearman:", best_row["best_spearman"])
print("Best Optuna mean MSE:", best_row["best_mse"])
print("Best params:", final_best_params)

# =========================================================
# 10. Refit final model on all chromosomes / all controls
# =========================================================
